...
public class ExeBaseBall {
	...
	/* function : 정수 리스트가 주어지면 해당 정수 리스트에 min~max 사이의 값을 
 *            스캐너를 통해 count개 입력받아 제대로 입력했는지 알려주는 메소드 
 *            0 : 제대로 입력, 1 : 범위잘못, -1 : 중복 
 * parameter : 리스트, 범위, 갯수, 스캐너 =>
 *             List<Integer> list, int min, int max, int count, Scanner scan 
 * return type : 0 또는 1 또는 -1 => 정수 => int 
 * title : inputList                        
 */
 	public static int inputList(List<Integer> list, int min, int max, int count, Scanner scan) {
		...
		while(i < count) {
			...
			useSet.add(tmp);
			list.add(tmp);
			//boolean OutofBounds = tmp >= min && tmp <= max ? isOutofBounds : true;
			if(tmp < min || tmp > max) {
				isOutOfBounds = true;
			}
			i++;
	...
	/*function : 두 정수 리스트가 주어지면 같은 번지에 있는 숫가가 몇개 같은지 알려주는 메소드  
	 * parameter : 두 정수 리스트 => List<Integer> list1, List<Integer> list2
	 * return type: 같은 번지에 있는 숫자 중 같은 숫자의 갯수 => 정수 => int 
	 * title: getStrike
	 */
		...
		/*function : 두 정수 리스트가 주어지면 다른 번지에 있는 숫가가 몇개 같은지 알려주는 메소드  
		 * parameter : 두 정수 리스트 => List<Integer> list1, List<Integer> list2
		 * return type: 다른 번지에 있는 숫자 중 같은 숫자의 갯수 => 정수 => int 
		 * title: getBall 
		 */
